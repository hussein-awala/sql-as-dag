...
"""
Demo: GROUP BY compiled from SQL, written to an Apache Iceberg table.

Same compiled two-stage GROUP BY as ``example_compiled_groupby``, but the sink is the Iceberg
connector (a local SQLite catalog + filesystem warehouse here). Each final partition writes a
Parquet data file and the ``finalize`` step commits them to the table in one snapshot.

Requires the iceberg extra::

    pip install "sql-as-dag[iceberg]"

Run it locally with::

    airflow dags test sql_as_dag_iceberg_groupby
"""
...
DAG_ID = "sql_as_dag_iceberg_groupby"

_DEMO_DIR = Path(tempfile.gettempdir()) / "sql_as_dag_iceberg"
_WAREHOUSE = _DEMO_DIR / "warehouse"
...
dag = dag_from_stages(
    ...
    sink=Sink(
        ...
        {
            "catalog_name": "demo",
            "uri": f"sqlite:///{_DEMO_DIR}/catalog.db",
            "warehouse": _WAREHOUSE.resolve().as_uri(),
            "identifier": "sql_as_dag.orders_by_customer",
        },
    ),
    schedule=None,
    start_date=datetime(2026, 1, 1),
    catchup=False,
    tags=["sql_as_dag", "example", "iceberg"],
)
