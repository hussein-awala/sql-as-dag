...
"""
Demo: a hash-shuffle INNER equi-join compiled from SQL.

``SELECT orders.customer_id, orders.amount, customers.name FROM orders JOIN customers ON
orders.customer_id = customers.id`` compiles to three stages: each side is scanned and
shuffled by its join key into N buckets, and the join stage joins one bucket from each side.
...
DAG_ID = "sql_as_dag_join"

_DEMO_DIR = Path(tempfile.gettempdir()) / "sql_as_dag_join"
_ORDERS = [
